...
const VendorProfile = () => {
  ...
  const shop = {
    ...
    name: "Bloom & Bean",
    description:
      "A cozy corner cafe specializing in artisanal coffee and fresh pastries. We source our beans ethically and bake all pastries in-house daily.",
    rating: 4.8,
    reviewCount: 127,
    phone: "(555) 123-4567",
    email: "hello@bloombean.com",
    website: "www.bloombean.com",
    openHours: [{ day: "Monday - Friday", hours: "7:00 AM - 7:00 PM" }],
    social: [{ platform: "Instagram", handle: "@bloomandbean", url: "#" }],
  ...
  const renderSocialIcon = (platform) => {
    switch (platform) {
      case "Contact Us":
        return <LinkIcon size={20} className="text-pink-500" />;
      default:
        return null;
    }
  };
          ...
          <div className="bg-gray-50 rounded-lg p-6">
            ...
            <div className="space-y-3">
              {shop.openHours.map((schedule, index) => (
                <div
                  key={index}
                  className="flex justify-between items-center py-2 border-b last:border-0 border-dashed border-gray-200"
                >
                  <span className="font-medium text-gray-700">
                    {schedule.day}
                  </span>
                  <span className="text-gray-600 bg-white px-3 py-1 rounded-md">
                    {schedule.hours}
                  </span>
                </div>
              ))}
